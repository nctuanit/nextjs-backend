...
import './globals.css';
export const metadata: Metadata = { title: 'Config | next-js-backend', description: 'ConfigModule · ConfigService · .env · TypeBox schema validation' };
      ...
      <div className="layout">
        <aside className="sidebar">
          <div className="sidebar-header">
            ...
            <div className="sidebar-title">11 · Config</div>
            <div className="sidebar-desc">ConfigModule · .env validation</div>
          </div>
          <nav className="sidebar-nav">
            <div className="nav-group-label">Features</div>
            {[{ label: 'ConfigModule.forRoot()', desc: '.env file loading' }, { label: 'ConfigService.get()', desc: 'Type-safe env access with defaults' }, { label: 'TypeBox validation', desc: 'Schema-validate env at startup' }].map(f => (
            ...
            {['GET /api/config/all', 'GET /api/config/env', 'GET /api/config/get'].map(r => (
          ...
          <div style={{ padding: '1rem 1.25rem', borderTop: '1px solid var(--border)' }}><span className="badge badge-blue">Sample 11/13</span></div>
        </aside>
